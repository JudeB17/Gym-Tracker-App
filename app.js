...
function App() {
  ...
  const startSession = useCallback(() => {
    ...
    setDraft({
      ...
      entries: day.items.map(it => {
        ...
        return { key: it.key, name: ex.n, t: ex.t, note: "",
  ...
  const addExerciseToDraft = useCallback(exKey => {
    ...
    setDraft(d => ({ ...d, entries: [...d.entries, { key: exKey, name: ex.n, t: ex.t, note: "", target: null, sets: [blankSet(ex.t)] }] }));
  }, [exByKey]);
  ...
  return (
    <Shell>
      ...
      <div style={{ padding: "0 14px 90px" }}>
        ...
        {tab === "history" && (
          viewSession
            ? <SessionDetail session={viewSession} onBack={() => setViewSession(null)} onDelete={deleteSession} exByKey={exByKey} />
...
function DraftView({ draft, setDraft, onSave, onDiscard, lastForExercise, exByKey, onAddExercise }) {
  const setEntry = (key, fn) => setDraft(d => ({ ...d, entries: d.entries.map(e => e.key === key ? fn(e) : e) }));
  const rmEntry = key => setDraft(d => ({ ...d, entries: d.entries.filter(e => e.key !== key) }));
  ...
  return (
    <div>
      <div style={{ ...card, display:"flex", gap:10, alignItems:"center" }}>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:16, fontWeight:700 }}>{draft.dayName}</div>
          <div style={{ fontSize:11, color:C.dim }}>{draft.date} · long-press ⠿ to reorder</div>
        </div>
        <div style={{ width:120 }}>
...
function ExerciseCard({ entry, setEntry, rmEntry, prev, ex, dragHandle }) {
  ...
  const addSet = () => setEntry(entry.key, e => ({ ...e, sets: [...e.sets, { ...(e.sets[e.sets.length-1] || blank) }] }));
  const rmSet = i => setEntry(entry.key, e => ({ ...e, sets: e.sets.filter((_, j) => j !== i) }));
  const upd = (i, k, v) => setEntry(entry.key, e => ({ ...e, sets: e.sets.map((s, j) => j===i ? { ...s, [k]: v } : s) }));
  const setNote = v => setEntry(entry.key, e => ({ ...e, note: v }));
  ...
  return (
    <div style={card}>
      <div style={{ display:"flex", alignItems:"center", gap:4 }}>
        ...
        <button onClick={() => rmEntry(entry.key)} style={{ background:"transparent", border:"none", color:C.dim, cursor:"pointer", fontSize:13 }}>remove</button>
      </div>
...
function SessionDetail({ session, onBack, onDelete, exByKey }) {
